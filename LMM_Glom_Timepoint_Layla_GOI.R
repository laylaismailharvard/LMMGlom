...
mts.2.3.5.glom <- read.csv("C:/Users/Laptop Admin/Desktop/Slice culture data/Quantile Normalized datasets/mts.2.3.5.glom.quantilenorm.data.csv",
                           row.names = 1)
head(mts.2.3.5.glom)
...
goi <- results[results$Significance == TRUE, "Gene"]



#heatmap

glom.gois<-mts.2.3.5.glom[goi,]

Annot_df <- data.frame(
  Timepoint = character(length = ncol(glom.gois)),
...
Annot_df$Timepoint<-sub(".*_(T[0-9]+)_.*", "\\1",colnames(glom.gois))
...
ordered.glom.data<-glom.gois[,order(Annot_df$Treatment, Annot_df$GlomLocation)]
...
tub.gois<-mts.2.3.5.tub[goi,]

Annot_df <- data.frame(
  Timepoint = character(length = ncol(tub.gois)),
  Treatment = character(length = ncol(tub.gois)),
  DonorID = character(length = ncol(tub.gois)),
  Location = character(length = ncol(tub.gois)),
  Histology = character(length = ncol(tub.gois))
)

Annot_df$Timepoint<-sub(".*_(T[0-9]+)_.*", "\\1",colnames(tub.gois))
Annot_df$Treatment<-sub(".*_(.*?)_.*", "\\1", colnames(tub.gois))
Annot_df$DonorID<-sub("^([A-Za-z0-9]+)_.*", "\\1",  colnames(tub.gois))
Annot_df$Location <- sub("^[^_]+_[^_]+_[^_]+_([^_]+)_.*", "\\1", colnames(tub.gois))
Annot_df$Histology <- sub("^[^_]+_[^_]+_([^_]+)_.*", "\\1", colnames(tub.gois))



row.names(Annot_df) <- colnames(tub.gois)
...
ordered.tub.data<-tub.gois[,order(Annot_df$Treatment, Annot_df$Location)]
...
All.gois<-all.T16vT0.Glom.Tub.compare[total.sig.genes,]


Annot_df <- data.frame(
  Timepoint = character(length = ncol(All.gois)),
  DonorID = character(length = ncol(All.gois)),
  Location = character(length = ncol(All.gois)),
  Histology = character(length = ncol(All.gois))
)

Annot_df$Timepoint<-sub(".*_(T[0-9]+)_.*", "\\1",colnames(All.gois))
Annot_df$DonorID<-sub("^([A-Za-z0-9]+)_.*", "\\1",  colnames(All.gois))
Annot_df$Location <- sub("^[^_]+_[^_]+_[^_]+_([^_]+)_.*", "\\1", colnames(All.gois))
Annot_df$Histology <- sub("^[^_]+_[^_]+_([^_]+)_.*", "\\1", colnames(All.gois))



row.names(Annot_df) <- colnames(All.gois)
...
ordered.all.data<-All.gois[,order(Annot_df$Timepoint,Annot_df$Histology, Annot_df$Location,Annot_df$DonorID)]
